...
2. **Google Workspace email** — loretta@lollieandlovielou.com + hello@ alias. $7/mo Workspace Starter. TXT verification goes in Netlify DNS panel (not GoDaddy).
...
4. **Claude token dashboard** — bat file fix: malformed `set ANTHROPIC_ADMIN_API_KEY=` line. Add Norman AOL inbox + manual run button. ⬜
5. **Dashboard dismiss behavior** — clarify: does dismiss on job search pending page delete the item? ⬜
6. **Remaining GitHub strategy files** — mmm_trucking.md, job_search.md, family_governance.md, sops/norman_inbox_guard.md not yet built. ⬜
...
|---|---|---|
| heropage_lollieandlovielou.jpg | hero.jpg | Hero — nose-to-nose kiss |
| lunch.jpg | restaurant.jpg | Our Story — EXIF rotation fixed |
| B2.jpeg | houndstooth.jpg | Strawberry — cropped head-to-boots |
| carshow1-3.jpeg | carshow1-3.jpg | The Car Show |
...
### Homepage Emotional Order
1. Hero — kissing shot + title + 3-line subheading
2. Emotional Hook Band
3. Our Story — restaurant photo + perspective shift
4. Real Moments — Car Show (3-photo) + Strawberry
5. Don't Shrink Their World — 4-photo grid
6. The Series — 4 book cards
7. Resource Guide Band — gated CTA
8. About Loretta — cruise photo
9. Email Capture
10. Footer

---

## Printful Shop SOP — Delivered ✅
...
Store connection: Static HTML → recommend Etsy (free) or Shopify Starter ($5/mo) linked from Shop tab.
...
### Lux Stack — Live ✅
- Five accounts running via lux_launcher.py at `C:\Users\aserc\.lux\workflows\`
...
- Add Norman AOL + manual run button — next session
...
1. Load context URL → confirm GitHub memory working
2. Claude token dashboard — bat file fix + Norman AOL + manual run button
3. Dashboard dismiss behavior — job search pending page clarification
4. Build remaining GitHub strategy files — mmm_trucking, job_search, family_governance
5. Veritas positioning paragraph
6. Google Workspace email setup
7. Mobile review — lollieandlovielou.com
