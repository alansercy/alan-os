...
3. **Re-add `MMM_SHEETS_URL` to `.env`** — `outbound_campaign.py` had a hardcoded Apps Script deployment URL; it was scrubbed during the lux-os repo init. **Status (verified 2026-04-30):** still missing. `C:\Users\aserc\.lux\.env` currently has only `ANTHROPIC_API_KEY` (1 of 2 non-comment lines). No commented-out `MMM_SHEETS_URL` line exists to uncomment — the var was never re-added after the repo init. **Fix:** locate the Apps Script deployment URL (Drive → MMM Prospect Tracker → Apps Script → Deploy → Manage deployments) and add `MMM_SHEETS_URL=https://...` as a new line in `.env`.
...
5. **Investigate host `:5678` listener** (open diagnostic, 2026-04-30) — `netstat -an` from the host shows `127.0.0.1:5678 LISTENING`, but `curl http://localhost:5678/healthz` returns no response (no HTTP at root, or non-HTTP service). CLAUDE.md §2 says "the host machine cannot reach the VM's `localhost:5678`" — that statement is about VM-side n8n, but a listener on the host is unexpected. Possibilities: (a) stale Docker container or local n8n install, (b) SSH tunnel forwarding to the VM that doesn't speak HTTP at `/healthz`, (c) other service bound to 5678. **Check:** `Get-NetTCPConnection -LocalPort 5678 | Select-Object -ExpandProperty OwningProcess` then `Get-Process -Id <pid>`. Not blocking VIE V1 (Python-only on host); becomes relevant for any host-side n8n work.
